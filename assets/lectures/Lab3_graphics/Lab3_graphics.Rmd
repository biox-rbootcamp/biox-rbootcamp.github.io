...
philosophy implemented in the `R` package `ggplot2`.

Work through this lab by running all the R code to your computer and making sure 
that you understand the input and the output. 

Begin by watching the lecture on vizualization (link on the website): this will provide you with an introduction to the importance of plotting and give you an overview of the different plotting functions with ggplot2.

Once you're done, go through the following lab which we will discuss on Monday.
...
and installs any missing ones. Note that (in the 'Lab2_graphics.Rmd' file) 
...
To find out about what data each column of `births2006.smpl` dataset, 
holds, you can run the line below in your console:

```{r findout, eval = FALSE}
...
The number ofbirths per day of the week can be shown easily in a barplot,
...
Answer the following questions on [Canvas](https://canvas.stanford.edu) 
in quiz "Quiz 2: Graphics". You may NOT work with other students to answer 
the questions on Canvas. You will need a Stanford ID to log in to Canvas.
The questions will be in random order on Canvas.
