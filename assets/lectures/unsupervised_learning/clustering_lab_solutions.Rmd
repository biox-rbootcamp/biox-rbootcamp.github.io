...
__Question 1: Implementing k-means__
It is important for you to understand how k-means works.
k-means is an iterative procedure, the most common way of implementing is as follows:

1. Initialize randomly the k-centers to k points of your dataset
2. While convergence has not been reached (e.g, too many point assignments are changing or the number of iteration t is below 100 ), do:
  a. Compute the pairwise distance between each point and its centroid (this yields a N x k matrix)
  b. Assign each data point to its nearest centroid
  c. Update the centroid (mean of all the coordinates of the points that belong to that cluster)
  
Implement the k-means algorithm from scratch. Check it on the following dataset, and compute your algorithm's accuracy. 

```{r}

df = data.frame(x = c(rnorm(30,0,0.5), rnorm(30,4,1),rnorm(30,-2,0.5), rnorm(30,0,0.5)),
                y =  c(rnorm(30,0,0.5), rnorm(30,2,0.5), rnorm(30,-1,1),rnorm(30,3,0.5)),
...
k=4
n = nrow(df)
centers =cbind(df$x, df$y)[sample(1:n, 4, replace=FALSE),] ### init
D = matrix(0, n, k)
it = 0
while(it <100){
  D = as.matrix(pdist(cbind(df$x, df$y), centers))
  Z = sapply(1:n, function(i){which.min(D[i,])})
  centers = rbind(apply(df[Z==1,1:2], 2, mean), apply(df[Z==2,1:2], 2, mean),
                  apply(df[Z==3,1:2], 2, mean), apply(df[Z==4,1:2], 2, mean))
  it = it + 1
  df$imputed =as.factor( Z)

  ggplot(df)+
  geom_point(aes(x=x, y=y, colour=imputed)) +
  theme_bw()
}
df$imputed =as.factor( Z)
...
__Question 2__ Compare your algorithm's accuracy to the one of the algorithm implemented in Base R.

```{r}
library(caret)
Z2 = kmeans(df[,1:2], 4)
ggplot(df)+
  geom_point(aes(x=x, y=y), colour=as.factor(Z2$cluster)) +
  theme_bw()

print(confusionMatrix(as.factor(Z2$cluster), as.factor(Z)))
```

__Question 3__ Using a method of your choice, choose the optimal number of cluster for the mock dataset in question 1.

```{r}

library(cluster)
set.seed(110010101)
gaps <- clusGap(df[,1:2], kmeans, K.max=20)
...
__Question 4__ Apply hierachical clustering to the mock dataset. Where would you cut it?


```{r}
dist.df <- dist(df[,1:2])
tree.df <- hclust(dist.df, "ward.D2")

# Making a prettier dendrogram.
library(dendextend)
tree.df$labels <- seq_along(tree.df$labels)
dend <- as.dendrogram(tree.df, hang=0.1)
plot(dend)
```
