...
Introduction
In generalized linear models, the modeled response is a function of the mean of Y, not Y itself. This function is called a link function. The purpose of the link function is to 1) convert the response value from a range [0,1] in logistic and probit and $[0,+\infty]$ for Poisson to a value ranging from $[−\infty, + \infty]$, and 2) create a linear relationship with the predictor variables.
...
$$y=X\beta=\ln(\frac{\pi}{1−\pi})$$

where $\pi$ is the "success probability" that an observation is in a specified category of the binary Y variable. You can solve this for $\pi$ to get
...
$$L(\beta;y,X)=\Pi_{i=1}^n\pi_i^{y_i}(1−\pi_i)^{(1−y_i)}.$$

The log-likelihood is

l(\beta)=\sum_{i=1}^n(y_iXi\beta−log(1+exp(Xi\beta))).
...
I’ll just compare $\text{REMISS} \sim \text{LI}$. In a situation like this where there the relationship is bivariate, start with a visualization.
...
Fit a logistic regression in R using glm(formula, data, family = binomial) where family = binomial specifies a binomial error distribution. For simplicity, model $\text{REMISS} \sim \text{LI}$.
...
$\hat{y} = X\beta = ln(\frac{\pi}{1-\pi})$

Suppose LI is 1.0, then the log odds of REMISS is  $\hat{y} =−3.777+2.897=−0.880$.
...
$$\exp(\hat{y} )=\exp(X\betâ )=\frac{\pi}{1−\pi}.$$

In this case, the odds of REMISS when LI = 1 is $exp(\hat{y} )=0.415$. You can solve the equation for $\pi$ to get the probability.
$$\exp(\hat{y} ) = \exp(X\hat{\beta}) =\frac{\pi}{1−\pi}.$$

In this case, the probability of REMISS when LI = 1 is $\pi=0.293$. The predict() function for a logistic model returns log-odds, but can also return $\pi$ with parameter type = "response".
...
$$\theta  = \frac{\pi/(1-\pi) |_{X=X_1}}{\pi/(1-\pi) |_{X=X_0}} = \frac{\exp{X_1 \hat{\beta}}}{\exp{X_0 \hat{\beta}}} = \exp((X_1 -X_0) \beta)= \exp(\delta \beta)$$   


In this simple bi-variate case, increasing LI by 10% increases the odds of REMISS by a factor of `exp(0.1⋅2.897)=1.336`. The odds ratio can be calculated from the model using `oddsratio::or_glm()`, or by using the `tidy()` function with `exponentiate = TRUE`. Note that $delta$ is 1, so if you want to see a .1 increase, change the model from `y ~ x to y ~ I(x*10).`
...
$$Z = \frac{\hat{\beta}_i}{SE(\hat{\beta_i})}$$
...
Evaluate a logistic model fit with an analysis of deviance. Deviance is defined as -2 times the log-likelihood $−2\ell(\beta)$. The null deviance is the deviance of the null model and is analagous to SST in ANOVA. The residual deviance is analagous to SSE in ANOVA.
...
$$\ell(\beta)=\sum_{i=1}^n(y_iX_i\beta−\sum_{i=1}^n\exp(X_i\beta)−\sum_{i=1}^n\log(y_i!).$$
...
The predicted value $\hat{y}$  is the estimated log of the response variable,$ =ln(\lambda)$.
...
The deviance of the null model (no regressors) is 139.9. The deviance of the full model is 132.2. The pseudo-R2 is very low at .05. How about the RMSE?

```{r}

library(MLmetrics)
RMSE(predict(m2, type = "response"),  fire$injuries)
```
