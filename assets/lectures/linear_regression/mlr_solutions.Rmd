...
Watch the following videos, before going through the lab. Prepare two questions on the material covered to ask in the discussion session.

- [Multiple Linear Regression](https://www.youtube.com/watch?v=RUgi_Uc1fx0&feature=youtu.be)
- [Multiple Linear Regression (2)](https://www.youtube.com/watch?v=PujxgY7B2aA&feature=youtu.be)
- [Multiple Linear Regression (3)](https://www.youtube.com/watch?v=bhQ0obnUabM&feature=youtu.be)
- [Multiple Linear Regression (4)](https://www.youtube.com/watch?v=3zjMDsPhJnk&feature=youtu.be)
...
Please prepare the following two exercises. 
You'll have a little bit of time to discuss them with your fellow classmates in class, but we'll ask you to correct these exercises.

### Professor Salaries and Confounding Variables
Let us consider [a dataset from major United States universities](https://raw.githubusercontent.com/biox-rbootcamp/biox-rbootcamp.github.io/master/assets/data/professor_salaries.csv) that was collected as part of the university’s ongoing evaluation of salary differences between its male and female faculty.
The Professor Salary Data includes the following variables:
...
__Question #0__ Load and start exploring the data. 


```{r}
library(ggplot2); theme_set(theme_bw())
data = read.csv("https://raw.githubusercontent.com/biox-rbootcamp/biox-rbootcamp.github.io/master/assets/data/professor_salaries.csv", row.names = 1)
names(data)

ggplot(data, aes(x=rank, y=salary, colour=sex))+
  geom_boxplot()


ggplot(data, aes(x=yrs.since.phd, y=salary, colour=sex))+
  geom_point()

ggplot(data, aes(x=yrs.service, y=salary, colour=sex))+
  geom_point()
...
Perform a two-sample t-test to compare the average salaries of male and female professors. Do males or females have higher salaries? Could the observed difference be due to random chance? Can you claim the difference is due to discrimination by the university?

```{r}
women =  data %>%filter(sex== "Female")
men =  data %>%filter(sex== "Male")
print(t.test(x=women$salary, y=men$salary,var.equal = FALSE))

```

__Question #2__

Identify potential ``confounding'' variables in the data -- that is, other variables that can also explain the difference between the two groups. Briefly explain what makes the variable “Rank” a confounding variable? (Hint: you should directly reference the definition of confounding in your answer)

- It seems from the plots in question 0 that the women professors in this dataset tend to be younger/ less experienced (lower number of years after PhD or seniority), which is correlated with the data/
...
```{r}
summary(lm(salary ~ sex, data=data))
```

*Solution:* The association between the salary and the gender is significant, as underlined by the coefficient in the regression: in average, men tend to earn 14,088 dollars more than women.
...
```{r}
summary(reg.mod <- lm(salary ~ sex + rank, data=data))

```

*Solution:* The gender coefficient is no longer significant under this model. This is consistent with the boxplots that we plotted in Q0.

__Question #6__

Rank is not the only confounding variable in these data, we could also argue that years.since.phd and discipline should be considered as confounders. Why is this the case? When building regression models, we typically add variables one-by-one and take time to understand what happens with each new addition. For this question we will add yrs.since.phd to the model from Questino #5; fit a regression model that uses “Sex”, “Rank”, and “yrs.since.phd” to predict “Salary”. Interpret each of the regression coefficients in this model.

```{r}
summary(reg.mod2 <- lm(salary ~ sex + rank + yrs.since.phd, data=data))
```

*Solution* This model gives us the effect of the gender on salary, accounting for the rank and years after PhD. In particular, the rank variable is the only one that comes out significant. Its interpretation is the following: within a certain gender group (Male/Female), for a given number of years after the PhD, Assist. Profs. earn in average 14,012 dollars less than Associate Profs. Full Professors earn 33,623 dollars more than Associate Profs.

__Question #7__

Use ANOVA to compare the fit of the model from Question #6 with the fit of the model from Lab Question #5. In your lab write-up, set up an ANOVA table summarizing this comparison. Check your work (the F-statistic and p-value) using the output from Question #6. Based upon your ANOVA test, does adding the predictor “yrs.since.phd” significantly improve the model?

*Solution* The null that we are testing here is whether the model from Question 6 is significantly different from Q5 -- ie, does the inclusion of “yrs.since.phd” improve anything? It seems that by running the ANOVA, the answer is no.
...
For Question #8, construct a graph of “yrs.since.phd” versus the residuals of the model from Lab Question #6 (the model containing “Sex”, “Rank”, and “yrs.since.phd” as predictors)s. Does the smoothed line show a curved pattern in the residuals?

*Solution* 
```{r}

df = data.frame(yrs.since.phd = data$yrs.since.phd,
                residuals = reg.mod2$residuals)
ggplot(df, aes(x=yrs.since.phd, y=residuals)) +
  geom_point() +
  geom_smooth(method="lm")
```
We observe a nice funnel plot: the greater the numberof years after the PhD, the higher the variance of the residuals. Hence, the homoscedasticity (same variance) assumption that linear regression makes is violated.
...
```{r}
data$yrs.since.phd2 = data$yrs.since.phd^2
summary(reg.mod3 <- lm(salary ~ sex + rank + yrs.since.phd+ yrs.since.phd2, data=data))

df = data.frame(yrs.since.phd = data$yrs.since.phd,
                residuals = reg.mod3$residuals)
ggplot(df, aes(x=yrs.since.phd, y=residuals)) +
  geom_point() +
  geom_smooth(method="lm")

anova(reg.mod2, reg.mod3)
```
...
*Solution* They are certainly extremely correlated, thus it might be useless -- we can use one or the other.

```{r}
ggplot(data, aes(x=yrs.service, y=yrs.since.phd ))+
  geom_point()
...
```{r}
summary(reg.mod4 <- lm(salary ~ sex + rank + yrs.since.phd+ yrs.service, data=data))
...
*Solution* The model that seems to fit the best is the first one (using just sex and rank). The gender coefficient has a none significant impact on the salaries.

### Predicting Cancer Survival

The [Breast Cancer Data](https://raw.githubusercontent.com/biox-rbootcamp/biox-rbootcamp.github.io/master/assets/data/Breast_Cancer_Data.csv) come from a study of breast cancer patients. I’ve subset the original data to only include patients who ended up experiencing death or a recurrence of their cancer. This is done for convenience sake, and this sort of subsetting isn’t optimal as there are better ways to analyze these data using more complex statistical methods. Nevertheless, we will analyze the subset using regression modeling.
...
cancer = read.csv("https://raw.githubusercontent.com/biox-rbootcamp/biox-rbootcamp.github.io/master/assets/data/Breast_Cancer_Data.csv")

ggplot(cancer)+
  geom_point(aes(x=PR, y=Time, colour="PR"))+
  geom_point(aes(x=ER, y=Time, colour="ER"))

ggplot(cancer)+
  geom_point(aes(x=Nodes, y=Time, colour="Time"))+
  geom_point(aes(x=Age, y=Time, colour="Age"))

ggplot(cancer)+
  geom_boxplot(aes(x=Grade, y=Time))

ggplot(cancer)+
  geom_boxplot(aes(x=Menopause, y=Time)) 
...
summary(lm(Time ~PR, data=cancer))
summary(lm(Time ~PR, data=cancer %>%filter(PR<900)))
...
For Question #15, create a new variable by applying a log-transformation to PR, then compare the R2 value of the simple linear regression model using the log-transformed variable with the R2 value of the simple linear regression model using the original, un-transformed variable (Hints: You might need to add a small positive value to each subject in your log-transformation formula; Also, you should be sure that subject #127 has been added back to your data for these comparisons)

```{r}
cancer$logPR = log(cancer$PR +1)
summary(lm(Time ~ logPR, data=cancer ))
ggplot(cancer)+
   geom_point(aes(x=logPR, y=Time))
```

__Question #17__ Plot the confidence intervals associated to your regression line.
...
For the final question,  use your knowledge to construct a model that you think will be most predictive of cancer survival time.
