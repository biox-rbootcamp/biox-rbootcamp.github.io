...
```{r}
data <- read.csv("data/mock_poisson_data.csv", sep=";")
```
...
__Question 2__ Aggregate the data to consider the total number of admissions per week. Does there seem to be evidence (from the graph) to support this claim?
...
Try to do the analysis for each ``disease''. This might be a little tricky, because there are a lot of them...

__Question 5__ How would you test  the different diseases?
