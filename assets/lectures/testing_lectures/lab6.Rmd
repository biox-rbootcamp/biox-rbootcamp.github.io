...
Now that we are comfortable with R and the fundamentals of plotting, we can  focus on starting to tackle the core question in data analysis: how do we extract patterns from large arrays of numbers, how do we make sense from large arrays of numbers? From here on, we will thus be focusing on __making sense of data from experiments__.
...
- Once you're done, work through the following lab.
...
Code the Monte Carlo simulation described above.

Click on the arrow and folded code chunks to reveal the answer
```{r, cache=TRUE}
library(ggplot2)
...
1. Suppose you sample 20 numbers from a normal distribution with mean = 75, and standard deviation =5. The mean of your sample is 76. Then, you take another sample of the same size, from the same distribution, and the mean of your second sample is 77. The mean difference is +1 (or -1, depending on how you take the difference)
...
**Answer**: Yes, it is inside the range of observed values--- in rounghly 50 \% of cases, the difference in means is at least 1.
```{r}
...
Watch the [video here](https://crumplab.github.io/statistics/t-tests.html) on the t-test.
...
- Another video on hypothesis testing
