...
1. Generate 100  simulated datasets of 150 p-values (let's say these are genes):
   - for each simulation, the first 140 genes expressions for all the subjects are sampled from the null: the observations from group A and B should be sampled from a normal variable with mean 2 and variance 1.
    - for each simulation, the last 10 gee expressions are sampled from the alternate hypothesis: the observations from group A should be sampled from a normal variable with mean 2 and variance 1, and the ones from group B have mean 2.5 and variance 1.
    
```{r}


delta_mean = sapply(1:100, function(x){
  sampleA = matrix(rnorm(50 * 150, 2, 1), 50, 150) 
  sampleB = cbind(matrix(rnorm(50 * 140, 2, 1), 50, 140), matrix(rnorm(50 * 10, 2.5, 1), 50, 10)
                  )
  sapply(1:150, function(i){t.test(sampleA[,i],sampleB[,i], alternative="two.sided" )$p.value})
})
delta_mean = t(delta_mean)
```
   
2. For each simulated dataset, compute the t-test associated to the means for group 1 and 2.

```{r}
detected = which(delta_mean < 0.05,arr.ind = TRUE)
false_discoveries = sapply(1:140, function(x){length(which(detected[,2] == x))})/100
true_discoveries = sapply(141:150, function(x){length(which(detected[,2] == x))})/100
```
3. Apply the Bonferroni correction (you can use the `p.adjust` function in R). Which p-values are significant?

```{r}
bonferonni = delta_mean
for(i in 1:100){
  bonferonni[i,] = p.adjust(delta_mean[i,], method = "bonferroni")
}
detected = which(bonferonni < 0.05,arr.ind = TRUE)
false_discoveries = sapply(1:140, function(x){length(which(detected[,2] == x))})/100
true_discoveries = sapply(141:150, function(x){length(which(detected[,2] == x))})/100
```
