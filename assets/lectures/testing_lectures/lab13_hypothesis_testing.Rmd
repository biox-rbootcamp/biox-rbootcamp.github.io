...
What hypothesis testing quantifies is thus a probability: it is the probability to observe a statistic (here, the difference in means) as extreme (or big) as we do under chance alone. This probability is known as the _p-value for your test_.
...
Step 2. Find a distribution for your statistic under the null
...
By way of example, let's focus on a very common test : the t-test. Watch the following [video here](https://crumplab.github.io/statistics/t-tests.html) on the t-test.
...
Indeed, suppose you have observations $X_1, \cdots X_n$ for the control group in your experiment, and $Y_1, \cdots Y_n$ for the treatment group. You want to measure the existence of a difference between the two groups.

Step 1. test-statistic : $T = |\bar{X} - \bar{Y}|$. Under the null, there is no difference between $\bar{X}$ and $\bar{Y}$ and they are sampled from the same distribution.

Step 2. Null distribution: by the central limit theorem:
...
Step 3. Compute the associated p-value. For reference, the t-distribution has the following shape.
