...
<img src="http://web.stanford.edu/class/bios221/images/TheMatrix.jpg" alt="Decompose1" align="middle" style="width: 600px;"/>
...
Diabetes Data, the first 5 variables are numeric and the last is a
factor variable encoding the specific patient group of the
subjects. /Users/susan/Dropbox/summer14/STAMPS/Lect_Multi/diabetesggplot.png

Various scales
======================

Transform the data one choice is to make all the variables, or
columns of the matrix have the same standard deviation, this will put
all the variables on the same footing in terms of their variance.

```{r, eval=FALSE,echo=FALSE}
require(GGally)
diabetef=data.frame(diabetes[,-6],Group=factor(diabetes[,6]))
ggpairs(diabetef,colour='Group',alpha=0.5)
```

We also make sure that the means of all columns are zero, this is called
centering .
...
====================
Invented
in 1901 by Karl Pearson as a way to reduce a two variable scatterplot to
a single coordinate.
...
variables at once. It is called Principal Component Analysis
(abbreviated PCA). (Useful books with relevant chapters are Flury for
an introductory account and Mardia for a more mathematical approach).
...
PCA is primarily a
visualization technique which produces maps that show the relations
between the variables in a useful way. We will use some geometrical
notions such as projections which take points in higher dimensions
and projects them down onto lower dimensions.
...
Linear Algebra
==================
PCA is a linear technique (based on linear algebra) and thus
particularly tractable.

There are also non-linear methods, which can be useful. 

We will see in
the next chapter where we study multidimensional scaling methods that
there are even ‘non metric methods’ that can plot and capture much more
general relationships and show data in a useful way not just by
projecting them onto a plane linearly.

The *raison d’être* for multivariate analyses is connections or
associations between the different variables. If the columns of the
...
we lose all the information about the second one. There are actually
...
in orange.
(/Users/susan/gitbiobook/BioBook/Chap8-IntroMultivariate/figure/chap8-Reg2.png)

A line that minimizes in both directions
----------------------------------------

```{r PCAmin,echo=FALSE}
p=ggplot(athletes,aes(x=weight, y=disc))+geom_point(size=3,shape=21)
pca <- princomp(athletes[,c(3,7)])
bp <- pca$loadings[2,1] / pca$loadings[1,1]
ap <- pca$center[2] - bp * pca$center[1]
ppca=p+geom_abline(intercept=ap,slope=bp, col="purple",linetype=1,lwd=3)
preg=ppca+ geom_abline(intercept=-a2/b2,slope=1/b2, col="green",linetype=1,lwd=2) +  
                    geom_abline(intercept=a,slope=b, col="blue",lwd=2)
preg+geom_segment(aes(x=weight, xend=reg2$fitted, y=disc, yend=disc), 
            linetype=2,colour="orange")+geom_segment(aes(x=weight, xend=weight, y=disc, 
            yend=reg1$fitted), linetype=2,colour="red") +
            geom_abline(intercept=ap,slope=bp, col="purple",linetype=1,lwd=3)
```
...
Good Projections
======================
![image](http://web.stanford.edu/class/bios221/images/chamgris.jpg)
...
![Many Choices have to made during PCA
processing.](http://web.stanford.edu/class/bios221/images/smallorgacp1.png)
...
# 
# <img src="/Users/susan/gitbiobook/BioBook/Chap8-IntroMultivariate/figure/SVD-mosaicXplot1.png" alt="Decompose1" style="width: 400px;"/>
# 
# Areas are proportional to the  entries
# 
# <img src="/Users/susan/gitbiobook/BioBook/Chap8-IntroMultivariate/figure/SVD-mosaicXplot2.png" alt="Decompose2" style="width: 400px;"/>
# 
# Looking at different possible margins
# 
# <img src="/Users/susan/gitbiobook/BioBook/Chap8-IntroMultivariate/figure/SVD-mosaicXplot3.png" alt="Decompose3" style="width: 400px;"/>
```
...
prows=apply(HairColor,1,sum)/sum(HairColor)
pcols=apply(HairColor,2,sum)/sum(HairColor)
...
$X$ in short hand as:
$$ X = USV', V'V=I, U'U=I, S \mbox{ diagonal matrix of singular values, given by the {\tt d} component in the R function}$$
...
ggplot() + geom_path(data = corcir, aes(x = x, y = y), colour = "gray65") + 
  ...
  geom_vline(xintercept = 0, colour = "gray65") + xlim(-1.1, 1.1) + ylim(-1.1, 1.1) + labs(x = "PC1 Axis", 
...
======================================
```{r VanillaGGplot,out.width="500px"}
##  PCA Example
require(ade4)
require(ggplot2)
...
You need to plot these values before you decide how many axes are
